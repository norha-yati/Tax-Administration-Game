...
.confetti-piece { position:absolute; width:8px; height:8px; border-radius:2px; animation: confetti-fall 1.5s ease forwards; }
 ...
 <body class="h-full">
  <div id="app" class="h-full w-full overflow-auto" style="background:#0a0f1e;">
   <!-- HOME SCREEN -->
   <div id="screen-home" class="h-full flex flex-col items-center justify-center p-6 text-center">
    <div class="slide-up max-w-lg w-full">
     ...
     <div class="grid grid-cols-1 gap-3 mb-8">
      <button onclick="startGame('quiz')" class="option-btn flex items-center gap-4 p-5 rounded-2xl text-left" style="background:#111827; border:1px solid #1e293b;">
       <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0" style="background:#1e3a5f;">
        <i data-lucide="help-circle" style="color:#38bdf8; width:24px; height:24px;"></i>
       </div>
       <div>
       ...
       <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0" style="background:#1e3a2f;">
        <i data-lucide="shuffle" style="color:#4ade80; width:24px; height:24px;"></i>
       </div>
       <div>
        <div class="font-bold text-base" style="color:#e2e8f0;">
         Match Game
       ...
       <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0" style="background:#3a1e3a;">
        <i data-lucide="briefcase" style="color:#c084fc; width:24px; height:24px;"></i>
       </div>
       <div>
        <div class="font-bold text-base" style="color:#e2e8f0;">
         Tax Scenarios
   ...
   <div id="screen-quiz" class="h-full flex flex-col p-4 md:p-6 hidden">
    <div class="flex items-center justify-between mb-4">
     <button onclick="showScreen('home')" class="flex items-center gap-1 text-sm" style="color:#64748b;"><i data-lucide="arrow-left" style="width:16px;height:16px;"></i> Back</button>
     <div class="mono text-sm" style="color:#94a3b8;"><span id="q-current">1</span>/<span id="q-total">10</span>
     </div>
     <div class="flex items-center gap-1 mono text-sm font-bold" style="color:#fbbf24;"><i data-lucide="star" style="width:14px;height:14px;"></i> <span id="q-score">0</span>
     </div>
    </div>
   ...
   <div id="screen-match" class="h-full flex flex-col p-4 md:p-6 hidden">
    <div class="flex items-center justify-between mb-4">
     ...
     <div class="flex items-center gap-1 mono text-sm font-bold" style="color:#4ade80;"><i data-lucide="check-circle" style="width:14px;height:14px;"></i> <span id="m-matched">0</span>
     </div>
    </div>
   ...
   <div id="screen-scenario" class="h-full flex flex-col p-4 md:p-6 hidden">
    <div class="flex items-center justify-between mb-4">
     ...
     <div class="flex items-center gap-1 mono text-sm font-bold" style="color:#c084fc;"><i data-lucide="star" style="width:14px;height:14px;"></i> <span id="s-score">0</span>
     </div>
    </div>
   ...
   <div id="screen-results" class="h-full flex flex-col items-center justify-center p-6 text-center hidden">
    <div class="max-w-md w-full slide-up">
     ...
     <div class="flex gap-3">
      <button onclick="showScreen('home')" class="flex-1 py-4 rounded-xl font-bold" style="background:#1e293b; color:#e2e8f0;">Home</button> <button id="results-retry" class="flex-1 py-4 rounded-xl font-bold" style="background:#38bdf8; color:#0a0f1e;">Play Again</button>
     </div>
    </div>
...
const quizQuestions = [
  ...
  {topic:"IRB Functions",q:"Under SAS, what advantage does the IRB gain administratively?",options:["IRB must hire more staff","IRB's administrative burden of raising assessments is reduced","IRB collects more taxes automatically","IRB has more control over taxpayers"],answer:1,explain:"SAS shifts the computation responsibility to taxpayers, reducing IRB's workload in raising individual assessments and freeing resources for audit and enforcement."},
  ...
  {topic:"Amendment of Tax Return",q:"When amending a tax return resulting in additional tax, what items must be included?",options:["Just the additional income","Additional tax, penalties (if applicable), and any interest","Only the recalculated total tax","A new bank account for payment"],answer:1,explain:"When amending a return with additional tax payable, the taxpayer must include the additional tax amount, applicable penalties (10%), and any interest charges."},
  ...
  {topic:"Types of Assessments",q:"An increased assessment resulting from a successful appeal is made when the Director General:",options:["Wants to reduce tax","Discovers that the tax assessed is less than it should be after appeal","Receives a refund claim","Closes an audit with no findings"],answer:1,explain:"Following an appeal, an increased assessment is raised when the original assessment is found to be less than it should have been."},
  {topic:"Types of Assessments",q:"A reduced assessment resulting from a successful appeal is issued when:",options:["More tax is owed","The taxpayer appeals and the original assessment is found to be excessive","The taxpayer fails the audit","A new tax law is passed"],answer:1,explain:"Following an appeal, a reduced assessment is made when the original assessment is found to be excessive."},
  {topic:"Types of Assessments",q:"A protective assessment is raised when:",options:["The taxpayer is cooperative","Precautionary when facts are uncertain and revenue needs protection","Tax has been fully paid","The audit is complete with no issues"],answer:1,explain:"A protective assessment is raised as a precautionary measure when there's uncertainty about facts or legal interpretation, to protect government revenue within the statutory time limit."},
  ...
  {topic:"Fines & Penalties",q:"Tax evasion under Section 114 can result in:",options:["Warning letter only","Fine of RM1,000 to RM20,000 and/or imprisonment up to 3 years, plus 300% penalty","Only a 10% penalty","Community service"],answer:1,explain:"Section 114 provides for a fine of RM1,000 to RM20,000 and/or imprisonment up to 3 years, plus a special penalty of 300% of the tax undercharged."},
  {topic:"Fines & Penalties",q:"An employer who fails to deduct and remit PCB commits an offence and may face:",options:["No consequence","A fine of RM200 to RM20,000 and/or imprisonment up to 6 months","Only a warning","Fine of RM50"],answer:1,explain:"Under Section 120(1), failure to deduct and remit PCB is an offence carrying a fine of RM200–RM20,000 and/or imprisonment up to 6 months."},
  {topic:"Fines & Penalties",q:"When amending a tax return resulting in additional tax, what items must be included?",options:["Just the additional income","Additional tax, penalties (if applicable), and any interest","Only the recalculated total tax","A new bank account for payment"],answer:1,explain:"When amending a return with additional tax payable, the taxpayer must include the additional tax amount, applicable penalties (10%), and any interest charges."},
...
const matchSets = [
  ...
  {title:"Types of Assessments",pairs:[
    ...
    {term:"Increased",def:"Result of appeal — previous assessment was too low"},
    {term:"Reduced",def:"Result of appeal — previous assessment was excessive"},
    {term:"Protective",def:"Precautionary when facts are uncertain"},
  ]},
  {title:"Key Deadlines",pairs:[
    {term:"30 April",def:"Form BE filing deadline"},
    {term:"30 June",def:"Form B filing deadline"},
    {term:"15th of next month",def:"PCB/MTD remittance deadline"},
  ...
  {title:"Types of Offences",pairs:[
    {term:"Late Filing",def:"Penalty of 10% increase on tax payable (Section 112)"},
    {term:"Negligent Return",def:"Penalty equal to tax undercharged (Section 113)"},
    {term:"Tax Evasion",def:"Fine RM1K–RM20K, jail up to 3 yrs, 300% penalty (Section 114)"},
    {term:"Failure to Remit PCB/MTD",def:"Fine RM200–RM20K, jail up to 6 months (Section 120)"},
  ]},
];
...
function startGame(type) {
  if(type==='quiz') {
    quizState.questions = shuffle(quizQuestions).slice(0, Math.min(15, quizQuestions.length));
...
lucide.createIcons();
 ...
 <script>(function(){function c(){var b=a.contentDocument||a.contentWindow.document;if(b){var d=b.createElement('script');d.innerHTML="window.__CF$cv$params={r:'9eea216c05e18066',t:'MTc3NjU4MzA1Ny4wMDAwMDA='};var a=document.createElement('script');a.nonce='';a.src='/cdn-cgi/challenge-platform/scripts/jsd/main.js';document.getElementsByTagName('head')[0].appendChild(a);";b.getElementsByTagName('head')[0].appendChild(d)}}if(document.body){var a=document.createElement('iframe');a.height=1;a.width=1;a.style.position='absolute';a.style.top=0;a.style.left=0;a.style.border='none';a.style.visibility='hidden';document.body.appendChild(a);if('loading'!==document.readyState)c();else if(window.addEventListener)document.addEventListener('DOMContentLoaded',c);else{var e=document.onreadystatechange||function(){};document.onreadystatechange=function(b){e(b);'loading'!==document.readyState&&(document.onreadystatechange=e,c())}}}})();</script></body>
</html>
